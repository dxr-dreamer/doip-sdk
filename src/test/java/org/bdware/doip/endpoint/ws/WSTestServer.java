...
package org.bdware.doip.endpoint.ws;
...
public class WSTestServer {
    static Logger LOGGER = LogManager.getLogger(WSTestServer.class);
    ...
    public static void run(int port) throws InterruptedException {
        ...
        try {
            DoipListenerConfig tlsinfo = new DoipListenerConfig("ws://127.0.0.1:" + port+"/wsdoip", "2.1");
            infos.add(tlsinfo);
        } catch (Exception e) {
            e.printStackTrace();
        }
